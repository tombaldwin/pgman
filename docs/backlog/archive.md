# pgman backlog — archive

Completed pgman work, moved out of `BACKLOG.md` so that file holds only
open items. Newest at the top within each section.

## Done

Historical record. Newest at the top within each section.

### 2026-09-02 grooming catch-up
- **psql backslash command routing — extras.** `\l` (database list) and
  `\x` (expanded row view) now ship alongside `\d` / `\dt` / `\dn` / `\?`
  / `\q` / `\timing` (see Done → Editor); `\df` (functions) is still
  missing and stays open in `BACKLOG.md`. *(done 2026-09-02, see
  CHANGELOG)*
