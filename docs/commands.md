...
| `--discovered` | In `--batch` mode with no `--dsn`, accept the single data source discovered in this checkout. Without it a lone discovered candidate is refused — nothing found in the working tree connects without a deliberate act, and batch has no keypress to offer. Zero or several candidates still need `--dsn`. |
