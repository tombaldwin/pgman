...
//! `qname="shop"` in the local file. The right pick is
//! `postgres://postgres@h:5432/shop`, not the URL's empty-path default.
//!
...
<project version="4">
  ...
  <component>
    ...
    fn parse_local_extracts_user_and_schema_mapping_databases() {
        ...
        // .local file. The dbname `shop` is what we actually want.
        let xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<project version="4">
  <component name="dataSourceStorageLocal">
    <data-source name="postgres@localhost" uuid="dc30c5be-e7c9-4f5f-a119-79aec0403c13">
      ...
      <schema-mapping>
        ...
        assert_eq!(meta.databases, vec!["shop"]);
    }
  ...
  <component>
    ...
    fn expand_to_dsns_uses_schema_mapping_when_url_has_no_path() {
        ...
        // user-name is in the local file, schema-mapping pins `shop`.
        let src = IntellijDataSource {
        ...
        assert_eq!(suffix.as_deref(), Some("shop"));
        assert_eq!(dsn.host, "localhost");
        assert_eq!(dsn.port, 5432);
        assert_eq!(dsn.dbname, "shop");
